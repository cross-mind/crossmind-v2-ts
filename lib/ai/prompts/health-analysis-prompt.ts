...
5. **assignNodeToZone**: 为节点分配或调整区域归属，可以将未分配的节点分配到合适的区域
...
1. 使用 **viewFrameworkZones** 获取框架的整体结构和节点分布
   - **注意**: 工具会返回两类节点：
     - **zones[]**: 已分配到各区域的节点列表
     - **unassignedNodes[]**: 未分配到任何区域的节点列表
2. 识别哪些区域有节点、哪些为空
3. **检查未分配节点**: 如果有 unassignedNodes，这些节点需要特别关注
   - 使用 **viewNode** 查看未分配节点的详细内容
   - 根据节点内容判断应该归属的区域
   - 使用 **assignNodeToZone** 将未分配节点分配到合适的区域
4. 对于有节点的区域，选择代表性节点使用 **viewNode** 查看详情
5. **不要一次性查看所有节点**，而是根据需要逐步深入
...
- 节点分布是否均衡？
- 是否有明显的空白区域？
- **关键**: 是否存在未分配节点？未分配节点说明这些内容没有明确归属
...
**建议类型**:
- "add-node": 缺少关键节点时
  - **必须提供 actionParams**: "{ zoneName: \"区域名称\", suggestedContent: \"具体的节点内容\" }"
  - zoneName: 节点应该放置的区域名称（如"问题"、"解决方案"、"关键指标"等）
  - suggestedContent: 为新节点准备的具体内容，而非描述性文本
  - title: 简短的建议标题（如"建立关键指标测量体系"）
  - description: 为什么需要这个节点的说明（如"在'关键指标'区域添加节点，定义能够衡量业务健康度的核心指标"）
- "add-tag": 需要补充分类标签时
...
**处理未分配或位置错误的节点**:
- 当发现节点未分配或放置在错误的区域时，**直接使用 assignNodeToZone** 将其分配到正确位置
- 分配节点后，可以创建一个 "health-issue" 类型的建议说明该问题
...
createSuggestion({
  type: "add-node",
  title: "建立关键指标测量体系",
  description: "在'关键指标'区域添加节点，定义能够衡量业务健康度和增长的核心指标",
  reason: "当前框架缺少可量化的成功指标，无法有效追踪业务进展",
  priority: "high",
  actionParams: {
    zoneName: "关键指标",
    suggestedContent: "建议包含：\\n1. 获客指标：客户获取成本(CAC)、获客渠道效果\\n2. 活跃指标：日活跃用户(DAU)、用户参与度\\n3. 留存指标：用户留存率、流失率\\n4. 收入指标：客户生命周期价值(LTV)、月均收入(MRR)"
  }
})
\`\`\`
